...
class Dribbble {
    ...
    protected $_api_root      = 'https://api.dribbble.com/v1';
    protected $_token;
    ...
    public function getUserShots($username, $counter) {
        $endpoint = $this->_api_root . self::ENDPOINT_USERS . '/' . $username . self::ENDPOINT_SHOTS . '?access_token=' . $this->_token . '&per_page=' . $counter;

        return $this->_executeRequest($endpoint);
    }

    protected function _executeRequest($url) {
        $user_agent = "Dribbble API/PHP (App {$this->_token})";
